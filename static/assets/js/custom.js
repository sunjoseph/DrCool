...
jQuery(document).ready(function($) {
	    ...
	    scroll: {
		    ...
		    pauseOnHover: 'immediate',
	   		onBefore: function(oldItems, newItems) {				
	   			// This first one is going out
		        newItems.last().animate({opacity: 0}, {duration: 'slow'});		
		        oldItems.last().animate({opacity: 1}, {duration: 9000,easing: 'easeOutBack'});
		        // This last one is going out
		        oldItems.first().animate({opacity: 0}, {duration: 4000,easing: 'easeOutBack'});
		    }
	    },
	    items: {
	      width: 186,
	...
	// Set tooltip for phone number at header.
	$('.call-us').tooltip();
	$('.social-tooltip').tooltip();

	// Remove empty tag about us at footer
	$(".footer-center p").filter( function() {
    	return $.trim($(this).html()) == '';
	}).remove()

    //Remove intro text
    $('#wrapper > p:first-child').remove();
	...
	// Replace contact us
	$('#breadcrumb-menu-contact').text('Contact Us');	 

	//Check required fied form contact			
	$('form#formContact .wrap-fields textarea,input').blur(function(){		
      	if( this.value ) {
	...
	$('form#comment .wrap-fields input,textarea').blur(function(){			
	      if( this.value ) {
	        $(this).css('border','1px solid #ccc');
    		$(this).css('color','#333');
	      }  
